...
# Gripper closing angle during gripping
servo1 = 500
...
# Set the RGB light color on the extension board to match the tracked color
def set_rgb(color):
    if color == "red":
...
def move():
    ...
    # Coordinates for placing different colored blocks (x, y, z)
    coordinate = {
    ...
    while True:
        if __isRunning:
            if first_move and start_pick_up:  # When the object is detected for the first time
                ...
                setBuzzer(0.1)
                result = AK.setPitchRangeMoving((world_X, world_Y - 2, 5), -90, -90, 0)  # Run without specifying the running time parameter, adaptively determine the running time
                if result == False:
                    unreachable = True
                else:
                    unreachable = False
                ...
                first_move = False
                action_finish = True
            elif not first_move and not unreachable:  # If not the first time the object is detected
                set_rgb(detect_color)
                if track:  # If in the tracking phase
                ...
                if start_pick_up:  # If the object has not moved for a while, start gripping
                    ...
                    if not __isRunning:
                        continue
                    # Classify and place different colored blocks
                    ...
                    if not __isRunning:
                        continue
                    Board.setBusServoPulse(1, servo1 - 200, 500)  # Gripper opens, place the object
                    time.sleep(0.8)
...
def run(img):
    ...
    # If a recognized object is detected in a certain area, keep detecting that area until none
    ...
    if not start_pick_up:
        ...
        if area_max > 2500:  # If the largest area is found
            ...
            roi = getROI(box) # Get the roi region
            get_roi = True
            ...
            distance = math.sqrt(pow(world_x - last_x, 2) + pow(world_y - last_y, 2)) # Compare the last coordinates to determine if it has moved
